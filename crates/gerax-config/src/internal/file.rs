...
pub(crate) fn read(path: impl AsRef<Path>) -> ConfigResult<String> {
...
#[allow(dead_code)]
pub(crate) fn absolute(path: impl AsRef<Path>) -> ConfigResult<PathBuf> {
    std::fs::canonicalize(path).map_err(|source| ConfigError::Io {
        path: "<unknown>".into(),
