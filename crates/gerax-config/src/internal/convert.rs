...
#[cfg(feature = "toml")]
pub(crate) fn from_toml(value: toml::Value) -> ConfigResult<Value> {
    serde_json::to_value(value).map_err(|e| ConfigError::Deserialize(e.to_string()))
}

/// Converte uma estrutura YAML para JSON Value.
#[cfg(feature = "yaml")]
pub(crate) fn from_yaml(value: serde_yaml::Value) -> ConfigResult<Value> {
    serde_json::to_value(value).map_err(|e| ConfigError::Deserialize(e.to_string()))
}
...
/// }
/// ```
pub(crate) fn ensure_object(value: Value) -> ConfigResult<Value> {
    match value {
