...
Suporte a MongoDB para o framework Gerax.

## Dependências

```toml
gerax-mongodb = { path = "../crates/gerax-mongodb" }
```

## Visão Geral

Implementa `Connection` e `Repository<T>` para MongoDB, integrando-se a
`gerax-db` e `gerax-core`. A configuração pode ser carregada a partir de
variáveis de ambiente.

```
MongoDbConfig -> MongoDbRepository<T> -> Connection / Repository<T>
```
...
Repositório MongoDB para entidades que implementam `gerax_core::Entity`.
...
use gerax_mongodb::{MongoDbConfig, MongoDbRepository};
